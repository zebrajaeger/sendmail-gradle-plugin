...
public abstract class SendMailTask extends DefaultTask {
  ...
  public void resolveLatestVersion() {
    final SendMailExtension extension = (SendMailExtension) getProject().getExtensions()
        .findByName(SendMailPlugin.ID);
    if (extension == null) {
  ...
  private void sendMail(SendMailExtension extension) {
    ...
    Mailer mailer = MailerBuilder
        .withSMTPServer(
            ...
            smtp.getUser(),
            smtp.getPassword())
        .withTransportStrategy(smtp.getTls() ? TransportStrategy.SMTP_TLS : TransportStrategy.SMTP)
